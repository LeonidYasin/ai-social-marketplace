  ...
  for (const screenshot of screenshots) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      try {
        ...
        console.log(`   Слов найдено: ${result.data.words ? result.data.words.length : 0}`);
        console.log(`   Строк найдено: ${result.data.lines ? result.data.lines.length : 0}`);
        console.log(`   Блоков найдено: ${result.data.blocks ? result.data.blocks.length : 0}`);
