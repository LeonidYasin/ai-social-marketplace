...
class OCRIsolatedBot {
    ...
    async init(browserCount = 2) {
        console.log(`🧠 Инициализация изолированного OCR бота (${browserCount} браузеров)...`);
    ...
    async analyzeDOM(pageIndex) {
        ...
        try {
            const domInfo = await this.pages[pageIndex].evaluate(() => {
                ...
                // Ищем элементы пользователей
                const userElements = document.querySelectorAll('[class*="user"], [class*="profile"], [data-testid*="user"]');
                ...
                // Ищем элементы основного приложения (общие)
            ...
            console.log(`   - Кнопок email: ${domInfo.emailButtons.length}`);
            ...
            console.log(`   - Элементов сайдбара: ${domInfo.sidebarElements.length}`);
    ...
    async analyzeScreenState(pageIndex, description = '') {
        ...
        const state = this.determinePerfectState(allText, domInfo);
    ...
    determinePerfectState(allText, domInfo) {
        ...
        // Проверяем элементы пользователей
        const hasUserElements = domInfo && domInfo.userElements.length > 0;
        if (hasUserElements) {
            indicators.push('элементы пользователей');
        }

        // ИДЕАЛЬНАЯ ЛОГИКА ОПРЕДЕЛЕНИЯ СОСТОЯНИЯ
        ...
        // НОВАЯ ЛОГИКА: Если есть элементы приложения, но нет модальных окон - пользователь уже вошел
    ...
    async smartClickWithVerification(pageIndex, searchText, expectedStates, description = '') {
        ...
        // Проверяем, не находимся ли мы уже в одном из ожидаемых состояний
        if (Array.isArray(expectedStates)) {
            if (expectedStates.includes(beforeState.state.name)) {
                console.log(`✅ Браузер ${pageIndex + 1}: Уже находимся в ожидаемом состоянии "${beforeState.state.name}"`);
                return { success: true, alreadyInState: true };
            }
        } else {
            if (beforeState.state.name === expectedStates) {
                console.log(`✅ Браузер ${pageIndex + 1}: Уже находимся в ожидаемом состоянии "${expectedStates}"`);
                return { success: true, alreadyInState: true };
            }
        }
        ...
        for (const clickMethod of clickMethods) {
            ...
            if (clickResult.success) {
                ...
                const reachedExpectedState = Array.isArray(expectedStates) 
                    ? expectedStates.includes(afterState.state.name)
                    : afterState.state.name === expectedStates;
    ...
    async smartGuestLogin(pageIndex) {
        console.log(`\n🎯 Браузер ${pageIndex + 1}: Изолированный гостевой вход`);
        ...
        if (initialState.state.hasModals && initialState.state.hasGuestButtons) {
            ...
            // Пытаемся кликнуть по кнопке гостевого входа (ожидаем login_methods или main_app)
            const guestLoginResult = await this.retryAction(async () => {
                return await this.smartClickWithVerification(
                    pageIndex, 
                    'гость', 
                    ['login_methods', 'main_app'], 
                    'для гостевого входа'
                );
            });
            
            if (guestLoginResult.success) {
                // Если получили login_methods, нужно кликнуть по email
                if (guestLoginResult.actual === 'login_methods') {
                    console.log(`🔧 Браузер ${pageIndex + 1}: Открыто окно выбора способа входа, кликаем по email`);
                    
                    const emailResult = await this.retryAction(async () => {
                        return await this.smartClickWithVerification(
                            pageIndex, 
                            'email', 
                            'main_app', 
                            'для выбора email входа'
                        );
                    });
                    
                    if (emailResult.success) {
                        console.log(`✅ Браузер ${pageIndex + 1}: Email вход выполнен успешно`);
                        return { success: true, loginMethod: 'email' };
                    } else {
                        console.log(`❌ Браузер ${pageIndex + 1}: Email вход не выполнен`);
                        return emailResult;
                    }
                } else {
                    // Если сразу получили main_app
        ...
        // Если нет модального окна, но есть кнопки гостевого входа - пользователь уже вошел
        if (initialState.state.hasGuestButtons && !initialState.state.hasModals) {
            console.log(`✅ Браузер ${pageIndex + 1}: Пользователь уже вошел в систему (есть кнопки гостевого входа без модального окна)`);
            return { success: true, alreadyLoggedIn: true };
        }
        ...
        const loginButtonResult = await this.retryAction(async () => {
            return await this.smartClickWithVerification(
                pageIndex, 
                'войти', 
                ['guest_login_modal', 'login_methods'], 
    ...
    async checkUserInteraction(pageIndex) {
        console.log(`\n🔍 Браузер ${pageIndex + 1}: Проверка взаимодействия пользователей`);
        
        const state = await this.analyzeScreenState(pageIndex, 'user_interaction_check');
        
        // Анализируем элементы пользователей
        const userElements = state.domInfo.userElements;
        console.log(`📊 Браузер ${pageIndex + 1}: Найдено ${userElements.length} элементов пользователей`);
        
        // Ищем уникальные имена пользователей
        ...
        userElements.forEach(element => {
            if (element.text) {
                ...
                if (text.includes('пользователь') || text.includes('user') || text.includes('гость')) {
                    // Пытаемся найти уникальные идентификаторы
                    const matches = text.match(/(пользователь|user|гость)\s*(\d+|[а-яё]+)/gi);
                    if (matches) {
                        matches.forEach(match => userNames.add(match.trim()));
                    }
                }
            }
        });
        
        console.log(`👥 Браузер ${pageIndex + 1}: Уникальные пользователи: ${Array.from(userNames).join(', ')}`);
        
        // Проверяем, есть ли другие пользователи кроме текущего
        const hasOtherUsers = userNames.size > 1;
        
        if (hasOtherUsers) {
            console.log(`✅ Браузер ${pageIndex + 1}: Обнаружены другие пользователи`);
            return { success: true, userCount: userNames.size, users: Array.from(userNames) };
        } else {
            console.log(`⚠️ Браузер ${pageIndex + 1}: Другие пользователи не обнаружены`);
            return { success: false, userCount: userNames.size, users: Array.from(userNames) };
        }
    }

    async runIsolatedTest() {
        try {
            console.log('🧠 Запуск изолированного тестирования');
            ...
            // Шаг 2: Изолированный гостевой вход для всех браузеров
            console.log('\n📋 Шаг 2: Изолированный гостевой вход');
            const loginResults = [];
            ...
            // Шаг 3: Проверяем реальное мультипользовательское взаимодействие
            if (this.pages.length > 1) {
                console.log('\n📋 Шаг 3: Проверка реального мультипользовательского взаимодействия');
                
                // Проверяем, что пользователи видят друг друга
                for (let i = 0; i < this.pages.length; i++) {
                    const interactionResult = await this.checkUserInteraction(i);
                    
                    if (interactionResult.success) {
                        console.log(`✅ Браузер ${i + 1}: Обнаружены ${interactionResult.userCount} пользователей: ${interactionResult.users.join(', ')}`);
                    } else {
                        console.log(`⚠️ Браузер ${i + 1}: Пользователи не обнаружены (${interactionResult.userCount} пользователей)`);
                    }
                }
            }
            
            console.log('\n🎉 Изолированное тестирование завершено успешно!');
            return true;
            
        } catch (error) {
            console.error('❌ Ошибка в изолированном тестировании:', error.message);
            return false;
        }
    }
...
// Запуск изолированного бота
async function main() {
    const bot = new OCRIsolatedBot();
    
    try {
        await bot.init(2); // Запускаем 2 браузера с изолированными профилями
        const success = await bot.runIsolatedTest();
        
        if (success) {
            console.log('\n✅ Изолированное тестирование прошло успешно!');
        } else {
            console.log('\n❌ Изолированное тестирование завершилось с ошибками');
        }
