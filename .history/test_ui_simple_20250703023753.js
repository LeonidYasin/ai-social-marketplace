...
async function registerUser(page, email, password, pageName) {
  try {
    ...
    if (welcomeText) {
      console.log(`[${pageName}] На экране 'Добро пожаловать', ищем кнопку 'Войти в систему'...`);
      await page.screenshot({ path: path.join(screenshotsDir, `02a_welcome_${pageName}.png`) });
      
      // Сначала пробуем найти кнопку "Войти в систему"
      const loginBtn = await page.$x("//button[contains(., 'Войти в систему') or contains(., 'Войти')]");
      if (loginBtn.length > 0) {
        console.log(`[${pageName}] Нажимаем 'Войти в систему'...`);
        await loginBtn[0].click();
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        // Делаем скриншот после нажатия "Войти в систему"
        await page.screenshot({ path: path.join(screenshotsDir, `02c_after_login_click_${pageName}.png`) });
        
        // После нажатия "Войти в систему" должен появиться экран с социальными сетями
        // Ищем кнопку Email там
        const emailBtn = await page.$x("//button[contains(., 'Email') or contains(., 'email') or contains(., 'Регистрация по email')]");
        if (emailBtn.length > 0) {
          console.log(`[${pageName}] Нажимаем кнопку Email...`);
          await emailBtn[0].click();
          await new Promise(resolve => setTimeout(resolve, 1000));
        } else {
          console.log(`[${pageName}] Кнопка Email не найдена после 'Войти в систему'!`);
          // Попробуем найти любые кнопки на экране для диагностики
          ...
          console.log(`[${pageName}] Доступные кнопки:`, allButtons);
          await page.screenshot({ path: path.join(screenshotsDir, `02b_no_email_btn_${pageName}.png`) });
          return;
        }
      } else {
        console.log(`[${pageName}] Кнопка 'Войти в систему' не найдена!`);
        // Попробуем найти любые кнопки на экране для диагностики
        const allButtons = await page.evaluate(() => {
          return Array.from(document.querySelectorAll('button')).map(btn => ({
            text: btn.textContent,
        ...
        console.log(`[${pageName}] Доступные кнопки:`, allButtons);
        await page.screenshot({ path: path.join(screenshotsDir, `02b_no_login_btn_${pageName}.png`) });
        return;
      }
    }
    // --- Конец блока ---
